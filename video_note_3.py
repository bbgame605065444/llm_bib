...
# from moviepy.editor import VideoFileClip # Using the more specific import
from moviepy import *
import whisper
import torch
import json
...
class VideoProcessor:
    ...
    def extract_audio(self, video_path):
        ...
        try:
            ...
            # FIX: Use the 'logger=None' argument to prevent a crash related to the deprecated 'verbose' parameter.
            audio.write_audiofile(audio_path, logger=None, ffmpeg_params=["-v", "quiet"])
...
class GradioInterface:
    ...
    def upload_video(self, video):
        ...
        self.video_path = video.name if hasattr(video, 'name') else 'uploaded_video'
        
        # 获取第2分钟的预览帧
        frame = self.processor.get_frame_at_time(video.name, 120)  # 2分钟 = 120秒
        ...
        if frame is not None:
            ...
            frame = self.processor.get_frame_at_time(video.name, middle_time)
            if frame is not None:
                self.preview_frame = frame
                self.clicks = []
    ...
    def extract_audio_only(self, video):
        ...
        try:
            audio_path = self.processor.extract_audio(video.name)
    ...
    def process_video_full(self, video, interval, threshold, progress=gr.Progress()):
        ...
        video_filepath = video.name
        
        try:
            progress(0.1, desc="提取音频...")
            audio_path = self.processor.extract_audio(video_filepath)
            ...
            frames, frame_numbers = self.processor.extract_frames_with_roi(
                video_filepath, interval, self.processor.roi
            )
            ...
            # 重新创建音频文件供下载
            final_audio_path = self.processor.extract_audio(video_filepath)
