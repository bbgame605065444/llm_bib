...
class ArticleAgent:
    ...
    def search_and_collect_papers(self, keyword: str, max_results: int = Config.MAX_RESULTS) -> Tuple[List[Dict], str]:
        ...
        try:
            ...
            json_filename = os.path.join(self.output_dir, f"{keyword.replace(' ', '_')}_{timestamp}_papers.json")
    ...
    def analyze_paper_relevance(self, papers: List[Dict], keyword: str) -> Tuple[List[Dict], str]:
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            analysis_filename = os.path.join(self.output_dir, f"{keyword.replace(' ', '_')}_{timestamp}_analysis.json")
